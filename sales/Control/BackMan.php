...
class BackMan {
	...
	private function CheckMustPermiss($projectId) {
		$adminPermiss = $this->m_adminPermissDb->Get ( array (
				"equal" => array (
	...
	public function Login($post_name, $post_pass) {
		...
		unset ( $_SESSION ["type"] );
		// 取出账号密码对应的用户
		$admin = $this->m_adminDb->Get ( array (
				"equal" => array (
						"name" => $post_name,
	...
	public function ModAdmin($post_adminId, $post_phoneNumber, $post_mail) {
		...
		if ($this->m_userType != ADMIN_TYPE_SUPER)
			Util::ThrowException ( 1, "需要超级管理员权限" );
			// 修改管理员信息
		$admin = $this->m_adminDb->Mod ( array (
				"phoneNumber" => $post_phoneNumber,
				"mail" => $post_mail 
		), array (
				"equal" => array (
	...
	public function GetAdminList($get_name = "", $get_pageFrom = 0, $get_pageSize = 10) {
		...
		$admin = $this->m_adminDb->Get ( $where, $get_pageFrom, $get_pageSize );
		$adminCount = $this->m_adminDb->Count ( $where );
		global $ADMIN_TYPE_NAME;
		foreach ( $admin as $key => $value ) {
			$admin [$key] ["typeName"] = $ADMIN_TYPE_NAME [$admin [$key] ["type"]];
			$admin [$key] ["oper"] = "<a href='#' data='" . $value ['adminId'] . "' class='view'>查看</a>|" . "<a href='#' data='" . $value ['adminId'] . "' class='del'>删除</a>";
		}
		return array (
				"data" => $admin,
	...
	public function ModAdminPermissList($post_adminId, $post_permissList = array()) {
		...
		// 删除这个人原来的权限
		$this->m_adminPermissDb->Del ( array (
				array (
						"adminId" => $post_adminId 
				) 
		) );
		...
		if (count ( $addAdminPermissList ) != 0) {
			...
			$this->m_adminPermissDb->Add ( $addSql );
		}
		// 删除权限
		if (count ( $delAdminPermissList ) != 0) {
			...
			$this->m_adminPermissDb->Del ( $delSql );
		}
		return array ();
	}
	// 帐号管理
	...
	public function ModAccount($post_userId, $post_name, $post_phoneNumber, $post_identityNumber, $post_bankNumber, $post_remark, $post_state) {
		...
		if ($user [0] ['state'] == USER_STATE_VALID && ($post_state != USER_STATE_VALID_CANCEL && $post_state != USER_STATE_VALID))
			Util::ThrowException ( 1, "审核通过的用户只能进行注销" );
			// 修改指定的用户
		$this->m_userDb->Mod ( array (
				"name" => $post_name,
				"phoneNumber" => $post_phoneNumber,
	...
	public function GetAccountList($get_phoneNumber = "", $get_name = "", $get_state = "", $get_pageFrom = 0, $get_pageSize = 10) {
		...
		// 获取用户的消息列表
		$data = $this->m_userDb->Get ( $where, $get_pageFrom, $get_pageSize, array (
				"desc" => "modifyTime" 
		) );
		$dataCount = $this->m_userDb->Count ( $where );
	...
	public function GetDealList($get_projectName = "", $get_name = "", $get_phoneNumber = "", $get_middleManName = "", $get_pageFrom = 0, $get_pageSize = 10) {
		...
		if (! empty ( $get_projectName )) {
			...
			$project = $this->m_projectDb->Get ( array (
					"like" => array (
		...
		// 按照条件筛选出最终结果
		$deal = $this->m_dealDb->Get ( $where, $get_pageFrom, $get_pageSize, array (
				"desc" => "modifyTime" 
		) );
		$dealCount = $this->m_dealDb->Count ( $where );
		if (count ( $deal ) == 0) {
			return array (
					"data" => $deal,
	...
	public function GetDealInfo($get_dealId) {
		...
		// 获取状态转移的时间
		$dealState = $this->m_dealStateDb->Get ( array (
				"equal" => array (
						"dealId" => $get_dealId 
				) 
		) );
	...
	public function ModDealState($post_dealId, $post_state, $post_message = "") {
		...
		// 发送消息
		global $DEAL_STATE_NAME;
		$this->AddMessage ( $deal ["middleManId"], "你的订单编号为" . $deal ["dealId"] . "的状态已更新为\"" . $DEAL_STATE_NAME [$post_state] . "\"请注意订单信息变更。", MESSAGE_TYPE_PRIVATE );
		return array ();
	}
	// 项目管理
	public function AddProject($post_title, $post_description, $post_stock, $file_pictureUrl, $post_infoUrl) {
		...
		// 添加项目
		$insertId = $this->m_projectDb->Add ( array (
				array (
						"title" => $post_title,
		...
		// 添加权限
		$this->m_adminPermissDb->Add ( array (
				array (
						"adminId" => $this->m_userId,
						"projectId" => $insertId 
				) 
	...
	public function GetProjectList($get_title = "", $get_pageFrom = 0, $get_pageSize = 10) {
		...
		$where ["in"] ["projectId"] = $userPermiss;
		// 获取项目
		$projectList = $this->m_projectDb->Get ( $where, $get_pageFrom, $get_pageSize, array (
				"desc" => "modifyTime" 
		) );
		$dataCount = $this->m_projectDb->Count ( $where );
	...
	public function GetMessageList($get_name = "", $get_pageFrom = 0, $get_pageSize = 10) {
		...
		// 获取消息列表
		$message = $this->m_messageDb->Get ( $where, $get_pageFrom, $get_pageSize, array (
				"desc" => "modifyTime" 
		) );
		$messageCount = $this->m_messageDb->Count ( $where );
		// 获取消息的类型
		$userIds = array ();
	...
	public function GetDealAppealList($get_state = "", $get_pageFrom = 0, $get_pageSize = 10) {
		...
		$dealAppeal = $this->m_dealAppealDb->Get ( $where, $get_pageFrom, $get_pageSize, array (
				"desc" => "modifyTime" 
		) );
		$dealAppealCount = $this->m_dealAppealDb->Count ( $where );
	...
	public function GetDealAppealInfo($get_dealAppealId) {
		...
		$this->CheckMustLogin ();
		// 获取申诉数据
		$dealAppeal = $this->m_dealAppealDb->Get ( array (
				"equal" => array (
						"dealAppealId" => $get_dealAppealId 
				) 
		) );
	...
	public function ConfirmDealAppeal($post_dealAppealId, $post_message) {
		...
		$dealAppeal = $this->m_dealAppealDb->Get ( array (
				"equal" => array (
						"dealAppealId" => $post_dealAppealId 
				) 
		) );
		if (count ( $dealAppeal ) == 0)
