...
class MiddleMan {
	...
	public function AddDeal($post_userId, $post_projectId, $post_name, $post_phoneNumber, $post_area, $post_budget) {
		...
		// 添加订单
		$dealId = $this->m_dealDb->Add ( array (
				array (
						"projectId" => $post_projectId,
						"name" => $post_name,
	...
	public function GetMessageList($get_userId, $get_type, $get_pageFrom = 0, $get_pageSize = 10) {
		...
		// 获取其所有消息
		$messages = $this->m_messageDb->Get ( $where, $get_pageFrom, $get_pageSize, array (
				"desc" => "modifyTime" 
		) );
		$dataCount = $this->m_messageDb->Count ( $where );
	...
	public function ConfirmMessage($post_userId, $post_messageId) {
		...
		$this->CheckMustValidUser ( $post_userId );
		// 获取消息
		$message = $this->m_messageDb->Get ( array (
				"equal" => array (
	...
	public function GetProjectList($get_userId, $get_pageFrom = 0, $get_pageSize = 10) {
		...
		// 检查用户状态
		$this->CheckMustValidUser ( $get_userId );
		$projects = $this->m_projectDb->Get ( array (), $get_pageFrom, $get_pageSize, array (
				"desc" => "modifyTime" 
		) );
		$dataCount = $this->m_projectDb->Count ( array () );
		return array (
				'data' => $projects,
				'count' => $dataCount [0] ['count'] 
		);
