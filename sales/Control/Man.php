...
class Man {
	...
	public function RegisterRequest($post_userId, $post_phoneNumber, $post_identityNumber) {
		// 校验参数
		Util::CheckValid ( "手机号码", $post_phoneNumber, "digitchar|11,11" );
		// Util::CheckValid( "身份证号码",$post_identityNumber , "digitchar|15,18");
		if (strlen ( $post_identityNumber ) != 15 && strlen ( $post_identityNumber ) != 18)
			Util::ThrowException ( 1, "身份证号码只能为15位或18位" );
			// 检查是否有相同的用户
		...
		if (count ( $user ) != 0)
			...
			// 检查注册请求是否太频繁
		$register = $this->m_registerDb->Get ( array (
				array (
						"userId" => $post_userId 
				) 
		), 0, 1, array (
		...
		for($i = 0; $i != CHECKNUMBER_LENGTH; $i ++)
			...
			// 添加注册请求
		$registerId = $this->m_registerDb->Add ( array (
				array (
						"userId" => $post_userId,
						"phoneNumber" => $post_phoneNumber,
	...
	public function RegisterUser($post_userId, $post_name, $post_phoneNumber, $post_identityNumber, $post_identityUrl, $post_remark, $post_registerId, $post_checkNumber) {
		...
		if (strlen ( $post_identityNumber ) != 15 && strlen ( $post_identityNumber ) != 18)
			...
			// Util::CheckValid( $post_registerId , "integer|1,1000000000");
			// Util::CheckValid( $post_checkNumber , "digitchar|6,6");
			// 验证用户短信
		$register = $this->m_registerDb->Get ( array (
				"equal" => array (
	...
	public function UploadImage($file_imageUrl) {
		...
		return array (
				"data" => $newImageFileUrl 
		);
	}

	public function UploadImage2($post_imageUrl) {
		$newImageFileUrl = Util::AddPostFile ( $post_imageUrl, UPLOAD_FILE_TYPE, UPLOAD_FILE_MAX_SIZE, "/Img" );
		return array (
				"data" => $newImageFileUrl 
		);
	}
