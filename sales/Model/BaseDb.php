...
class BaseDb {
	...
	public function Count($where = array()) {
		$sql = $this->m_mysqlUtil->GenerateSelectSql ( $this->m_tableName, "count", array (), $where, - 1, - 1 );
		return MySql::GetInstance ()->Query ( $sql );
	}

	public function Get($where = array(), $pageFrom = -1, $pageSize = -1, $sortColumn = array()) {
		$sql = $this->m_mysqlUtil->GenerateSelectSql ( $this->m_tableName, "data", array (), $where, $pageFrom, $pageSize, $sortColumn );
		return MySql::GetInstance ()->Query ( $sql );
	}
	...
	public function Add($data) {
		...
		$sql = $this->m_mysqlUtil->GenerateInsertSql ( $this->m_tableName, $column, $values );
		MySql::GetInstance ()->Update ( $sql );
		return MySql::GetInstance ()->GetLastInsertId ();
	}

	public function Del($where) {
		$sql = $this->m_mysqlUtil->GenerateDeleteSql ( $this->m_tableName, $where );
		MySql::GetInstance ()->Update ( $sql );
	...
	public function Mod($data, $where) {
		...
		$sql = $this->m_mysqlUtil->GenerateUpdateSql ( $this->m_tableName, $data, $where );
		MySql::GetInstance ()->Update ( $sql );
		return MySql::GetInstance ()->GetAffectedRows ();
	}
}
