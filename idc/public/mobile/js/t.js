...
function load(){
	...
	if(page == "message"){
		...
		$("#page-customer #search_phoneNumber").blur(function(){
				$("#search_phoneNumber").css("text-align", "center");
				$("#search_phoneNumber").css("background", "url(./images/ico_search.png) no-repeat 5% 50% / auto 14px #fff");
		});
...
var customer_feed = '<li data="%s" onclick="click_customer_detail(this)">'
			...
			+'</div>'
			+'<p>%s</p>'
			+'<p><span>面积：%s</span><span>预算：%s</span></p>'
			+'</li>';
...
function get_customer_list(phoneNumber){
	...
	$.get("/json.php/MiddleMan/GetDealList", {userId: userId, pageFrom: pageFrom, pageSize: pageSize, phoneNumber: phoneNumber}, function(data){
		...
		for(var i in deal){
			...
			var row = sprintf(customer_feed, deal[i].dealId, deal[i].name ,deal[i].phoneNumber,	deal[i].projectName, deal[i].stateName, deal[i].area, deal[i].budget);
