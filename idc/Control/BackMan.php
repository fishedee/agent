...
public function ModAccount( $post_userId , $post_name , $post_phoneNumber ,$post_identityNumber , $post_bankNumber , $post_remark , $post_state){
	...
	if( $user[0]["state"] != $post_state ){
		...
		if( $post_state == USER_STATE_VALID ){
			...
			//审核未通过
			$newUserId = "cancel_".md5(uniqid().mt_rand(1,1000000));
			$this->m_userDb->Mod(
				array(
					"userId"=>$newUserId
				),
...
public function GetAccountListExcel( $get_phoneNumber = "" , $get_name = "" ){
	$data = $this->GetAccountList($get_phoneNumber,$get_name,$get_type,-1,-1);
	$excelData = $data['data'];
	$excelColumn = array(
		array(
			"id"=>"userCode",
...
public function GetAccountList( $get_phoneNumber = "" , $get_name = "" ,$get_pageFrom = 0 , $get_pageSize = 10 ){
	//目前只有金牌代理人一个角色
...
public function GetDealListExcel( $get_projectName = "", $get_name = "", $get_phoneNumber = "", $get_middleManName ="" ){
	$data = $this->GetDealList($get_projectName,$get_name,$get_phoneNumber,$get_middleManName);
	$excelData = $data['data'];
	$excelColumn = array(
		array(
			"id"=>"projectId",
...
public function ModDealState( $post_dealId , $post_state , $post_message = "" ){
	...
	if( $post_state == DEAL_STATE_PAYING ){
		Util::CheckValid( $post_message , "integer|1,1000000000");
		$this->m_dealDb->Mod(
			array(
